    ...
    def forward(self, x, y):
        ...
        if self.use_dual_path:
            if self.use_cross_att:
                # 选cross，则交叉qkv
                x, y = (
                    self.window_attention_x(q=x, k=y, v=y),
                    self.window_attention_y(q=y, k=x, v=x),
                )
            else:
                # 不选cross，则两路互不干扰
                x, y = (self.window_attention_x(q=x, k=x, v=x), self.window_attention_y(q=y, k=y, v=y))
            return x, y
        return self.window_attention_x(q=x, k=x, v=x)
