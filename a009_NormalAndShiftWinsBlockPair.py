...
from a005_BasicBlock import BasicBlock
...
class NormalAndShiftWinsBlockPair(nn.Module):
    ...
    def __init__(
        ...
        self.normal_window_block = BasicBlock(
        ...
        self.shifted_window_block = BasicBlock(
            in_out_dims=in_out_dims,
            num_heads=num_heads,
            dims_per_head=dims_per_head,
            window_size=window_size,
    ...
    def forward(self, x, y=None):
        ...
        if self.use_dual_path:
            x, y = self.normal_window_block(x=x, y=y)
            x, y = self.shifted_window_block(x=x, y=y)
            return x, y
        x = self.normal_window_block(x=x, y=None)
        return self.shifted_window_block(x=x, y=None)
