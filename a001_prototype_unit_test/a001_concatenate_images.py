...
def start():
    ...
    # torch读入图片后格式为(c, h, w)
    image_tensor_list = [torchvision.io.read_image(path) for path in path_list]

    # four_image_tensor的shape为(4, c, h, w)
    four_image_tensor = torch.stack(image_tensor_list)

    torchvision.io.write_jpeg(method_1(four_image_tensor), r".\test_1.jpg")
    torchvision.io.write_jpeg(method_2(four_image_tensor), r".\test_2.jpg")
...
def method_1(four_image_tensor):
    ...
    return four_image_tensor.reshape(c, 2 * h, 2 * w)
...
def method_2(four_image_tensor):
    ...
    """
    _, c, h, w = four_image_tensor.shape
    return einops.rearrange(tensor=four_image_tensor, pattern="(n_h n_w) c h w -> c (n_h h) (n_w w)", n_h=2, n_w=2)
