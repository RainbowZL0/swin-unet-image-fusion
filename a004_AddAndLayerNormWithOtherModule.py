...
from a007_utils import put_channel_dim_to_the_last_position, put_channel_dim_to_the_second_position


class AddAndLayerNormWithOtherModule(nn.Module):
    def __init__(self, normalized_shape: list, use_dual_path: bool, other_module: nn.Module):
        super().__init__()
    ...
    def forward(self, x, y):
        ...
        if self.use_dual_path or y is not None:
            ...
            x, y = my_layer_norm(x=x, layer_x=self.norm_layer_1, y=y, layer_y=self.norm_layer_2)
...
def my_layer_norm(x, layer_x, y=None, layer_y=None):
    ...
    x = layer_x(x)
    return put_channel_dim_to_the_second_position(x)
