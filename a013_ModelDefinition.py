...
class MyModel(nn.Module):
    def __init__(
        ...
        self.att_num_heads: int = att_num_heads
        self.att_dims_per_head_ratio: float = att_dims_per_head_ratio
        self.attention_drop_ratio: float = attention_drop_ratio
        self.linear_after_att_drop_ratio: float = linear_after_att_drop_ratio
        self.mlp_hidden_dims_ratio: int = mlp_hidden_dims_ratio
        self.mlp_activation_func: nn.Module = mlp_activation_func
        self.mlp_drop_ratio: float = mlp_drop_ratio
        self.final_layer_att_dims_per_head_ratio: float = final_layer_att_dims_per_head_ratio
        self.final_layer_conv_kernel_size: int = final_conv_layer_kernel_size
        self.final_layer_mlp_hidden_dims_ratio: int = final_layer_mlp_hidden_dims_ratio
        ...
        """实验最后一层"""
        # 最后的合并通道层，输出最终结果
        self.final_layer: nn.Sequential = self.get_final_layer()
    ...
    def get_final_layer(self) -> nn.Sequential:
    ...
    def generate_model_using_deque(self):
        ...
        # 包装为ModuleList类，然后返回
        encoder_module_list, decoder_module_list = (
            nn.ModuleList(encoder_que),
            nn.ModuleList(decoder_que),
        )
    ...
    def forward(self, in_x, in_y) -> torch.Tensor:
        ...
        # u-net部分forward
        x, y = in_x, in_y
        for i, m_list in enumerate(self.encoder_list):  # type: ignore
            m_list: nn.ModuleList
            for m in m_list:
                x, y = m(x=x, y=y)
            if i < len(self.encoder_list) - 1:
                self.u_net_intermediate_result_recorder.record((x, y))
        for j, m_list in enumerate(self.decoder_list):  # type: ignore
            if j > 0:
