...
class BasicBlock(nn.Module):
    ...
    def check_input_compatibility_with_option(self, x, y):
        ...
        if self.input_compatibility_with_cross_option is None:
            assert x is not None, "x should not be None"  # In any case, x must not be None.

            compatible = True
            # 如果选了dual_path，那么必须传入两个向量
            if self.use_dual_path and y is None or not self.use_dual_path and y is not None:
                compatible = False

            # 如果选的是cross_attr，那么x和y不可能是每个元素完全相同的
            x: torch.Tensor
            if self.use_cross_attr and y is not None and (x == y).all():
                compatible = False
            ...
            if not compatible:
                ...
                self.input_compatibility_with_cross_option = True  # 修改用于记录状态的属性
    ...
    def forward(self, x, y=None):
        ...
        if self.use_dual_path or y is not None:
            x, y = self.stage_1(x, y)
            x, y = self.stage_2(x, y)
            return x, y
        x = self.stage_1(x=x, y=None)
        return self.stage_2(x=x, y=None)
