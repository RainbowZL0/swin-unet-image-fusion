...
class WindowAttention(nn.Module):
    def __init__(
        ...
        self.qk_scale = self.dims_per_head**-0.5  # will be multiplied after the score is got.
        ...
        # 位置编码相关
        self.relative_position_bias_indices = self.get_initial_relative_position_indices()
    ...
    def initialize_feature_shape_hw(self, q):
        ...
        if self.training:  # 训练时只需记录形状一次，每个输入形状相同
            if not self.feature_shape_hw:  # 空tuple判定为False，用not转为True。所以空tuple时进入if
    ...
    def get_initial_relative_position_indices(self):
        ...
        In the training process, it stays unchanged. That's why it is registered as a buffer, not a parameter.
    ...
    def initialize_mask_for_cyclic_shift(self):
        ...
        assert self.feature_shape_hw is not None, """
        ...
        one_column, one_row = one_column.expand(expected_shape), one_row.expand(expected_shape)
    ...
    def operate_mask_scores_for_cyclic_shift(self, scores: Tensor):
        ...
        # 测试时也要次次初始化，因为每次输入图片形状不同，使用的mask的形状也不同，主要是每张图切成多少个窗口不同。
        # 虽然每个窗口大小是相同的。
        if self.mask_for_cyclic_shift.numel() == 0 or not self.training:
    ...
    def calculate_linear_projection_and_reshape_back(self, tensor: Tensor):
        ...
        return self.rearrange_1_reverse(tensor)
    ...
    def check_if_need_and_do_or_undo_cyclic_shift(self, tensor_list, mode: str) -> list:
        ...
        return [torch.roll(input=elem, shifts=(shift_size_h, shift_size_w), dims=(2, 3)) for elem in tensor_list]
    ...
    def forward(self, q, k, v):
        ...
        """cyclic shift，如果需要。"""
        q, k, v = self.check_if_need_and_do_or_undo_cyclic_shift(tensor_list=[q, k, v], mode="do")
        """计算"""
        ...
        return self.check_if_need_and_do_or_undo_cyclic_shift(tensor_list=[output], mode="undo")[0]
