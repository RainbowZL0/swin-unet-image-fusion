...
# from a007_utils import put_channel_dim_to_the_second_position, put_channel_dim_to_the_last_position
...
class PatchMergingAndLinearLayer(nn.Module):
    ...
    def do_patch_merging_for_one_tensor(self, feature: Tensor) -> Tensor:
        ...
        return einops.rearrange(
            tensor=feature,
            pattern="b c (n_wins_h n_points_h) (n_wins_w n_points_w) ->b (n_points_h n_points_w c) n_wins_h n_wins_w",
    ...
    def undo_patch_merging_for_one_tensor_2(self, feature: Tensor) -> Tensor:
        ...
        feature = feature.reshape(batch_size, n_points_h_per_win, n_points_w_per_win, -1, n_wins_h, n_wins_w)
        # () -> (batch_size, channels, n_wins_h, n_points_h_per_win, n_wins_w, n_points_w_per_win)
        feature = feature.permute(0, 3, 4, 1, 5, 2)
        # () -> (batch_size, channels, n_wins_h * n_points_h_per_win, n_wins_w * n_points_w_per_win)
        return feature.reshape(batch_size, -1, n_wins_h * n_points_h_per_win, n_wins_w * n_points_w_per_win)
    ...
    def build_func_list(self):
        ...
        if self.belongs_to_encoder:
            return func_list
        return [func_list[1], func_list[2], func_list[0], func_list[3]]
