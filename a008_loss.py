...
from kornia.losses import MS_SSIMLoss, PSNRLoss, ssim_loss
...
class MyLoss(nn.Module):
    ...
    def calcu_psnr_loss(
        ...
        return self.fus_ir_psnr_weight * fus_ir_loss + self.fus_vis_psnr_weight * fus_vis_loss
    ...
    def calcu_ssim_loss(
        ...
        return fus_ir_loss * self.fus_ir_ssim_weight + fus_vis_loss * self.fus_vis_ssim_weight
    ...
    def calcu_texture_loss(
        ...
        return torch.mean(torch.abs(result))
    ...
    def calcu_intensity_loss(
        ...
        return torch.norm(input=fusion_images - max_value, p=1, keepdim=False) / fusion_images.numel()
    ...
    def calcu_total_loss(
        ...
        """
        # calculate SSIM loss
        the_ssim_loss = self.calcu_ssim_loss(fusion_images, ir_images, vis_images) * self.ssim_scale

        # calculate texture loss
        texture_loss = self.calcu_texture_loss(fusion_images, ir_images, vis_images) * self.texture_scale

        # calculate intensity loss
        intensity_loss = self.calcu_intensity_loss(fusion_images, ir_images, vis_images) * self.intensity_scale

        psnr_loss = self.calcu_psnr_loss(fusion_images, ir_images, vis_images) * self.psnr_scale if self.use_psnr else torch.tensor(0.0)
        ...
        # 构造详细loss信息的字典
        loss_state_dict = dict(zip(key_list, value_list, strict=False))
    ...
    def calcu_history_mean_and_clear_and_save_to_mean_recorder(self) -> dict:
        ...
        tuples = zip(*record_stack_dict_values_list, strict=False)
        ...
        means_dict = dict(zip(key_list_for_means_dict, means, strict=False))
