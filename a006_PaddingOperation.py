...
class MyPadding(nn.Module):
    def __init__(
        ...
        # 如果属于encoder，则表示padding之前的shape。
        # 如果属于decoder，则表示反向padding之后的shape。
    ...
    def update_feature_shape_hw_after_padding(self):
        ...
        if self.padding_size[0] != 0 or self.padding_size[1] != 0:
            new_feature_shape_hw = tuple(i1 + i2 for i1, i2 in zip(self.feature_shape_hw, self.padding_size, strict=False))
            self.set_feature_shape_hw(new_feature_shape_hw=new_feature_shape_hw)

    def do_padding(self, x: Tensor, y):
        """
    ...
    def do_padding_for_one_tensor(self, tensor) -> Tensor:
        ...
        return functional.pad(
            ...
            input=tensor,
            pad=(0, padding_right, 0, padding_down),
            mode="reflect",
        )
